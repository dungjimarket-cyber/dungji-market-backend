...
class EmailVerification(models.Model):
    ...
    purpose = models.CharField(
        ...
        verbose_name='용도',
        help_text='password_reset: 비밀번호 재설정, signup: 회원가입, change_email: 이메일 변경'
    )
    
    # IP 주소 (보안용)
    ip_address = models.GenericIPAddressField(null=True, blank=True, verbose_name='IP 주소')
    
    # 추가 정보 (비밀번호 재설정용 username 등)
    additional_info = models.JSONField(
        default=dict, 
        blank=True, 
        verbose_name='추가 정보',
        help_text='인증 시 제공된 추가 정보 (username 등)'
    )
    ...
    def __str__(self):
        return f'{self.email} - {self.get_status_display()} ({self.created_at.strftime("%Y-%m-%d %H:%M")})'
    ...
    def save(self, *args, **kwargs):
        if not self.pk:
            ...
            if not self.verification_code:
                self.verification_code = self.generate_verification_code()
            if not self.expires_at:
                self.expires_at = timezone.now() + timedelta(minutes=5)  # 5분 후 만료
        super().save(*args, **kwargs)
    ...
    @classmethod
    def get_recent_verification(cls, email, purpose='password_reset'):
        """최근 유효한 인증 가져오기"""
        return cls.objects.filter(
            email=email,
            purpose=purpose,
