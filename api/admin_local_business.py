...
"""
                            ...
                            # OpenAI에 검증 요청
                            prompt = f"""
다음 업체가 "{category_name}" 업종에 정확히 맞는지 엄격하게 판단해주세요.

업체명: {business_name}
업종: {category_name}

**다음 중 하나라도 해당되면 반드시 "NO"로 답변하세요:**

1. 업종이 맞지 않는 경우
2. 공공기관인 경우 (세무서, 세관, 구청, 시청, 동주민센터, 경찰서, 소방서, 우체국 등)
3. 세무사/회계사 업종인데 "세무서"가 포함된 경우 (공공기관)

**청소 전문 업종의 경우:**
- "세탁", "세탁소", "빨래방", "드라이클리닝", "코인워시" 포함 → NO (세탁업은 청소업 아님)
- 집/사무실 청소가 아닌 경우 → NO
- 청소 서비스를 제공하는 업체만 YES

**이사 전문 업종의 경우:**
- "창고", "보관", "스토리지", "물류센터", "컨테이너" 포함 → NO (창고/물류는 이사업 아님)
- 이삿짐 운송/포장이 아닌 경우 → NO
- 이사 서비스를 제공하는 업체만 YES

이 업체가 해당 업종과 정확히 일치하고 위 조건에 하나도 해당하지 않으면 "YES", 조금이라도 의심스러우면 "NO"로 답변하세요.

예시:
- "스타벅스" + 업종 "변호사" → NO
- "김앤장 법률사무소" + 업종 "변호사" → YES
- "하남시청" + 업종 "회계사" → NO
- "강남세무서" + 업종 "세무사" → NO
- "다올창고" + 업종 "이사 전문" → NO
- "안전보관센터" + 업종 "이사 전문" → NO
...
- "드라이크리닝" + 업종 "청소 전문" → NO
- "24시 빨래방" + 업종 "청소 전문" → NO
- "깨끗한 청소 서비스" + 업종 "청소 전문" → YES
- "하나로 이삿짐센터" + 업종 "이사 전문" → YES
"""
