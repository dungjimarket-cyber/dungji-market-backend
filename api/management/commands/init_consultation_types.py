"""
업종별 상담 유형 초기 데이터 생성 커맨드
Usage: python manage.py init_consultation_types
"""
from django.core.management.base import BaseCommand
from api.models_local_business import LocalBusinessCategory
from api.models_consultation import ConsultationType


class Command(BaseCommand):
    help = '업종별 상담 유형 초기 데이터 생성'
    ...
    def handle(self, *args, **options):
        ...
        for category_name, types in self.CONSULTATION_TYPES.items():
            try:
            ...
            for idx, (type_name, icon, description) in enumerate(types):
                obj, created = ConsultationType.objects.update_or_create(
                    category=category,
                    name=type_name,
                    defaults={
                        'icon': icon,
        ...
        self.stdout.write(
            self.style.SUCCESS(
                f'상담 유형 초기화 완료: {created_count}개 생성, {updated_count}개 업데이트'
            )
        )
