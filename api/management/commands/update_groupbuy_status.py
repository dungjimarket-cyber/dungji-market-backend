    ...
    def handle(self, *args, **options):
        ...
        for groupbuy in voting_expired:
            ...
            if vote_results:
                ...
                # 공구 상태를 seller_confirmation으로 변경
                groupbuy.status = 'seller_confirmation'
                groupbuy.save()
                
                logger.info(f"공구 {groupbuy.id}의 투표가 종료되어 판매자 {winning_bid.seller.username}가 선정되었습니다.")
            else:
                ...
                if first_bid:
                    ...
                    groupbuy.status = 'seller_confirmation'
                    groupbuy.save()
                    logger.info(f"공구 {groupbuy.id}의 투표가 없어 첫 번째 입찰자를 자동 선정했습니다.")
                else:
                    # 입찰도 없으면 취소
                    groupbuy.status = 'cancelled'
                    groupbuy.save()
