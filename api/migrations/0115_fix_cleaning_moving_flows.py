...
CLEANING_MOVING_FLOWS = [
    ...
    # 청소: 추가 요청
    {
        ...
        'depends_on_options': ['cleaning'],
        'options': [
            {'key': 'aircon', 'label': '에어컨 청소 포함', 'icon': '❄️'},
            {'key': 'kitchen', 'label': '주방 집중 청소', 'icon': '🍳'},
            {'key': 'bathroom', 'label': '욕실 집중 청소', 'icon': '🚿'},
            {'key': 'window', 'label': '창문 청소 포함', 'icon': '🪟'},
            {'key': 'none', 'label': '기본 청소만', 'icon': '✅'},
            {'key': 'custom', 'label': '직접 입력', 'icon': '📝', 'is_custom_input': True},
        ]
    },
]
...
def populate_cleaning_moving_flows(apps, schema_editor):
    ...
    print(f'청소업체 카테고리 플로우 생성 완료 ({len(CLEANING_MOVING_FLOWS)}개)')
